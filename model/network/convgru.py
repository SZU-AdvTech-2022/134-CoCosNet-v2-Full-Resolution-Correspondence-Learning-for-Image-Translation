...
class BasicMotionEncoder(nn.Module):
    ...
    def forward(self,flow,corr):
        ...
        return torch.cat([out,flow],dim=1)  #64 - 2*candidate + 2*candidate
        pass
...
class BasicUpdateBlock(nn.Module):
    ...
    """
    net  hidden : b x 32  x h x w
    corr                              : b x 16   x h x w
    flow offset_x + offset_y          : b x 2*16 x h x w 
    """
    def forward(self,net,corr,flow):
        motion_features = self.encoder(flow,corr)
        inp = motion_features  #  b x 64 x h x w
        net = self.gru(net,inp) # b x 32 x h x w
        delta_offset_x,delta_offset_y = self.flow_head(net) # 2: b x 16 x h x w
        """
        net c = 32,hidden feature
